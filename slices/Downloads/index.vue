...
<template>
  <section id="documents" :ref="setRef" class="w-full min-h-screen py-[140px] px-[16px] md:px-[60px]"
    :class="slice.primary.light_background_color ? 'bg-secondary' : 'bg-primary'"
    :data-nav="slice.primary.light_background_color ? 'dark' : 'light'" data-section="documents">
    <div class="max-w-[1300px] m-[auto]">
      <div class="flex flex-col md:flex-row md:items-end w-full mb-[40px] md:mb-[135px]"
        :class="slice.primary.light_background_color ? 'text-primary' : 'text-secondary'">
      ...
      <div class="flex flex-wrap gap-20 justify-center md:justify-start"
        :class="{ 'flex-wrap gap-20': slice.primary.documents.length > 2 }">
        <div v-for="(item, index) in slice.primary.documents" :key="index" :class="{
          'flex flex-row gap-20 mb-[40px]': slice.primary.documents.length > 2,
          'w-1/2 md:w-1/2': slice.primary.documents.length <= 2
        }" class="mb-[40px] md:mb-[0px]" data-item>
          <PrismicLink :field="item.download_url" class="group">
            <figure class="relative">
              <figcaption
                class="absolute z-10 max-w-[297px] h-full p-[32px] md:px-20 md:py-30 text-primary flex flex-col justify-between">
            ...
            <div class="flex items-center justify-between mt-20 max-w-[297px]">
              <h4 class="uppercase tracking-[0.08em] text-xsm"
                :class="slice.primary.light_background_color ? 'text-primary' : 'text-secondary'">
                {{ item.download_text }}
              </h4>
              <div
                ...
                <div class="inline-flex relative overflow-hidden">
                  <div
                    ...
                    <svg class="w-15 h-15" viewBox="0 0 17 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <path d="M8.38281 14.9028L8.38281 1"
                        :stroke="slice.primary.light_background_color ? 'var(--primary-color)' : 'var(--secondary-color)'"
