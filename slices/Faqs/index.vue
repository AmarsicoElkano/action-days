...
<template>
  <section
    id="faqs"
    :ref="setRef"
    class="bg-primary w-full min-h-screen py-[100px] px-[16px] md:px-[60px]"
    data-section="faqs"
    data-nav="light"
  >
    <h1
      v-if="slice.primary.display_title"
      data-title
    ...
    <div
      class="max-w-[1300px] flex flex-col md:flex-row justify-around m-[auto] pt-80 gap-80"
      :class="{ 'pt-0': !slice.primary.display_title }"
    >
      <div class="md:w-1/4 text-secondary relative mb-3">
        <div
          v-for="(item, categoryIdx) in uniqueCategories"
          :key="categoryIdx"
          class="py-[20px] flex flex-row"
        >
          <button
            v-if="uniqueCategories.length > 1"
            :class="{
            ...
            class="flex flex-row cursor-pointer h-[70px] items-center"
            @click="toggleCategory(categoryIdx)"
          >
      ...
      <div id="faqs-qa" class="md:w-2/3 relative">
        <div
          v-for="(category, categoryIdx) in uniqueCategories"
          :key="categoryIdx"
        >
          <div v-show="activeCategoryIndex === categoryIdx">
            ...
            <div
              v-for="(question, questionIdx) in category.questions"
              :key="questionIdx"
              class="mb-3 py-[10px]"
              data-item
            >
              ...
              >
                ...
                >
                  <div
                    class="p-10 pt-8 text-sm leading-normal text-blue-gray-500"
                  >
                    <PrismicRichText
                      class="faqResponse"
