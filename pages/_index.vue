...
<template>
  ...
  <section class="bg-primary text-white">
    <div class="flex items-center">
      <div class="w-1/2 pt-[132px]">
        <PrismicImage
          class="w-full h-[auto] max-w-[617px]"
          :field="home?.data.main_image"
        />
      </div>
  ...
  >
    <div class="flex flex-row gap-[60px]">
      <h2
        class="text-titleSection uppercase w-9/12 max-w-[722px]"
        data-title
      >
        {{ home?.data.agenda_title }}
      </h2>
      <PrismicRichText
        class="w-1/2 pt-[170px]"
        :field="home?.data.agenda_text"
      />
    </div>
  </section>
  <!-- agenda schedule -->
  <section
    v-for="(agenda, idx) in agendaSlices"
    :key="idx"
    class="w-full h-auto bg-secondary"
    :class="{
  ...
  >
    <div class="w-full bg-secondary min-h-screen">
      <div class="grid-layout pt-[100px] relative">
        <PrismicImage
          :field="agenda.primary.bg_symbol"
        ...
        <div class="h-screen relative col-start-6 col-span-12">
          <div
            ref="masonry"
            class="masonry grid gap-[20px] relative"
          >
            <div class="content text-primary text-download uppercase">
              <div class="p-20 grid grid-cols-2 w-[720px] gap-20">
                <div
                  v-for="(el, idx) in agenda.primary.card_group"
                  :key="idx"
                  class="p-20 bg-white w-full max-w-[330px] flex flex-col"
                  :class="idx === 1 ? 'mt-20' : ''"
                >
                  <PrismicLink
                    :field="el.link"
  ...
  <section
    id="when&where"
    class="bg-primary w-full min-h-screen"
  >
    <div
      v-for=" (data, idx) in whenAndWhereSlice"
      :key="idx"
    >
      <PrismicImage
        :field="data.primary.bg_symbol"
        class="z-[10] absolute w-full h-full left-0"
      />
      <div class="flex flex-row gap-100 py-60 pl-40 pr-40">
        <PrismicImage
          :field="data.primary.image"
          class="sm:w-[450px] pt-10"
        />
        <div class="z-[20] flex flex-col gap-100 pt-40">
          <div
            v-for="(item, index) in data.primary.questions"
            :key="index"
  ...
  <section
    id="faqs"
    class="bg-primary w-full min-h-screen flex w-full py-80 justify-between px-[60px]"
  >
    <h1 class="w-1/3 text-center text-titleSection text-secondary text-uppercase">
      FAQs
    </h1>
    <div
      v-for=" (data, index) in faqsSlice"
      :key="index"
    >
      <div
        v-for=" (item, idx) in data.primary.faqs"
        :key="idx"
      >
          ...
          <div class="mb-0">
            ...
            >
              ...
              <img
                ...
                class="minusSign absolute right-0 hidden ml-80 "
              >
              <img
                ref="plusSign"
                src="/public/img/plus.svg"
                class="plusSign absolute right-0 ml-80 block"
              >
            </div>
          </div>
  ...
  <section class="w-full h-screen bg-primary relative py-20">
    <div v-if="downloadsSlice">
      <div
        v-for=" (data, idx) in downloadsSlice"
        :key="idx"
      >
        <div class="grid-layout items-center relative">
          ...
          <div class="col-start-2 col-span-10 md:col-start-7 md:col-span-5">
            <div class="md:flex">
              <div
                v-for="(item, index) in data.primary.documents"
                :class="{ 'ml-80': index != 0 }"
              >
                <PrismicLink
                  :field="item.download_url"
                  class="group"
                >
