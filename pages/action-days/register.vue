...
<template>
  <div>
    ...
    <section
      ref="hero"
      class="bg-primary min-h-screen pt-[150px] px-[16px] md:px-[60px] relative"
    >
      <img
        src="/public/img/ellipse.svg"
        class="absolute top-20 left-0 right-0 mx-auto"
      />
      <div class="text-secondary relative z-10">
        <div class="pl-60">
          <p
            class="uppercase text-highlight sm:text-titleSection_mb md:text-titleSection lg:text-titleSection font-bold invisible"
            data-item
          >
          ...
          <p
            class="uppercase text-highlight sm:text-titleSection_mb md:text-titleSection lg:text-titleSection font-bold flex flex-row py-15 invisible"
            data-item
          >
            <span
              class="text-base_mb md:text-detail uppercase md:w-[120px] mr-40"
              data-subtitle
            >
          ...
          <p
            class="uppercase text-highlight sm:text-titleSection_mb md:text-titleSection lg:text-titleSection font-black ml-[41px] invisible"
            data-item
          >
        ...
        <div class="flex flex-col items-end justify-center gap-60 pt-80">
          <div class="max-w-[600px] w-full h-full">
            ...
            >
              <PrismicLink
                v-if="register?.data.link_to_registration"
                :field="register?.data.link_to_registration"
    ...
    <!-- app stores -->
    <section
      class="bg-primary h-auto pb-80 pt-[50px] px-[16px] md:px-[60px] relative"
      v-if="register?.data.title_app.length"
    >
      <div
        class="border-[1px] border-white border-opacity-25 px-60 py-60 flex flex-row gap-40"
      >
        <PrismicImage
          v-if="register?.data.image_qr_app"
          :field="register?.data.image_qr_app"
        />

        <div class="flex flex-col leading-10">
          <PrismicRichText
            class="text-secondary ml-10 text-title_base"
            :field="register?.data.title_app"
          />
          ...
          <div class="flex flex-row gap-20 ml-80">
            ...
            >
              <PrismicLink
                v-if="register?.data.link_apple_store"
                :field="register?.data.link_apple_store"
                class="tracking-wider"
                >app store
              </PrismicLink>
            </button>
            ...
            >
              <PrismicLink
                v-if="register?.data.link_play_store"
                :field="register?.data.link_apple_store"
                class="tracking-wider"
                >play store
              </PrismicLink>
